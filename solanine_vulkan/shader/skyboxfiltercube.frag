...
layout (push_constant) uniform PushConsts {
    ...
    layout (offset = 76) float sunRadius;
	layout (offset = 80) float sunAlpha;
} consts;
...
void main()
{
	...
	vec2 isectPlanet = rsi(r0, ray_world, rPlanet);
		...
		vec3 color = atmosphere(
			...
			consts.lightDir.xyz,                  	// position of the sun
	...
	float _sunRadius = length(normalize(ray_world) - normalize(consts.lightDir.xyz));
	if(_sunRadius < consts.sunRadius)
	{
		_sunRadius /= consts.sunRadius;
		float smoothRadius = smoothstep(0, 1, 0.1 / _sunRadius - 0.1);
		color = mix(color, SUN_BASE_COLOR * 4.0, smoothRadius * consts.sunAlpha);
	}
